...
def get_fixed_maturity_data():
    initial_date = sql_db().get_query_response("Select min(timestamp) from amberdata.deltasurfacefloatingmaturities_hour")[0][0]
    data = sql_db().get_query_response(f"Select * from amberdata.deltasurfacefloatingmaturities_hour where timestamp >= {initial_date}")
    df = pd.DataFrame(data)
    # sort df by timestamp
    
    column_names = sql_db().get_query_response("Select column_name from information_schema.columns where table_name = 'deltasurfacefloatingmaturities_hour'")
    column_names = [i[0] for i in column_names]
    # drop dupplicate column_names_list 
    column_names = list(dict.fromkeys(column_names))
    print(column_names)
    df.columns = column_names
    df = df[['timestamp','expirationTimestamp','daysToExpiration','atm']]
    df['timestamp'] = pd.to_datetime(df['timestamp'], unit='ms')
    df.sort_values(by='timestamp', inplace=True)
    # df['expirationTimestamp'] = pd.to_datetime(df['expirationTimestamp'], unit='ms')
    # df.to_pickle(r"D:\Data\KIVC\To Animesh\to Mahindra\From animesh\floating_surface_pickle_mini.pkl")
    df.dropna(inplace=True)
    df['int_exp'] = df['daysToExpiration'].apply(lambda x: int(np.ceil(x)))
    df.reset_index(drop=True, inplace=True)
    pivot_df = df.pivot_table(index='timestamp', columns='int_exp', values='atm')
    pivot_df.columns = [f'{col}-atmIV' for col in pivot_df.columns]
    ...
    pivot_df.to_pickle(r"D:\Data\KIVC\To Animesh\to Mahindra\From animesh\fixed_atm_iv.pkl")
    df.to_pickle(r"D:\Data\KIVC\To Animesh\to Mahindra\From animesh\fixed_atm_raw.pkl")
    return pivot_df,df
...
def vol_calc(no_days,coin = "BTC"):
    if coin == "BTC":
        df = update_data()
        _,fixed_mat_data = get_fixed_maturity_data()
    elif coin == "MSTR":
        df = update_data_mstr()
    fixed_mat_data['hr'] = fixed_mat_data['daysToExpiration'].apply(lambda x: int(x*24))
    # convert 'hr' to int 
    df.reset_index(inplace=True)
    fixed_mat_data.reset_index(inplace=True)
    fixed_mat_data.sort_values(by='timestamp', inplace=True)
    # get list of latest timestamp  daysToExpiration column
    latest_timestamp = max(fixed_mat_data['timestamp'])
    list_expiry = list(fixed_mat_data[fixed_mat_data['timestamp']==latest_timestamp]['daysToExpiration'])
    # only save <=90 days data in list_expiry
    list_expiry_hrs = [int(i*24) for i in list_expiry if i <= 90]
    list_maturity_dates = list(fixed_mat_data[fixed_mat_data['timestamp']==latest_timestamp]['expirationTimestamp'])
    # sort list_matuirty_dates
    list_maturity_dates.sort()
    # only select <=90 days data in list_maturity_dates
    list_maturity_dates = list_maturity_dates[:len(list_expiry_hrs)]
    # df = pd.read_pickle("D:\\Data\\KIVC\\To Animesh\\to Mahindra\\From animesh\\pickle_mini.pkl")
    ...
    a = {}
    for exp in list_expiry_hrs:
        # create variable names wrt exp , example rolling_{exp}_day_vol
        a[exp] = hourly_volatility.rolling(window=exp).std() * np.sqrt(24*365)
    vol_df = pd.DataFrame(a)
    a = pd.DataFrame(a)
    
    vol_stats = a.rolling(window=no_days*24).agg(['mean', 'std', 'min', 'max'])
    # convert tuple columns to string columns
    vol_stats.columns = vol_stats.columns.map(lambda x: f'{x[0]}_{x[1]}')
    for period in list_expiry_hrs:
    ...
    df.dropna(inplace=True)
    df.to_pickle(f"D:\\Data\\KIVC\\To Animesh\\to Mahindra\\From animesh\\Fixed_{coin}VolCone_{no_days}.pkl")
    fixed_mat_data.to_pickle(f"D:\\Data\\KIVC\\To Animesh\\to Mahindra\\From animesh\\Atm_{coin}_fixed_mat.pkl")
    return df,fixed_mat_data

try:
    # atm_iv = get_fixed_maturity_data()
    vol_calc(182)
    vol_calc(365)
    vol_calc(1095)
...
schedule.every(1).hour.do(get_fixed_maturity_data)
