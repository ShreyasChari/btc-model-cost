...
import logging
logging.basicConfig(filename='VolPredictor.log', level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
...
def main():
    # Load old data
    data_old = pd.read_csv("Vol Prediction Data_new.csv", index_col=0)
    ...
    # drop to csv file
    data_merged.to_csv("Vol Prediction Data_new.csv")
    print(data_merged.tail(10))
    prediction_data = VolModel(data_merged)
    print(prediction_data)

    # save as csv
    prediction_data.to_csv("Vol Prediction Data_to_plot.csv")
    ...
    ax.legend()
    plt.savefig('Actual_vs_Predicted_DVOL.png')
